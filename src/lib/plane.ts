  ...
  return {
    ...
    async load(context) {
      ...
      try {
        ...
        logger.error(
          `Plane loader failed to list work items: ${errorMsg(error)}`,
        );
        return;
      }
      ...
      try {
        ...
        logger.error(
          `Plane loader failed to fetch work item properties: ${errorMsg(error)}`,
        );
        return;
      }
      ...
      if (!slugPropertyId) {
        logger.error(
          "Plane loader could not find a custom property named Slug.",
        );
        return;
      }
      ...
      for (const item of workItems) {
        ...
        try {
          rawSlug =
            (await client.fetchSlugValue(item.id, slugPropertyId))?.trim() ??
            "";
        } catch (error) {
          logger.error(
            `Plane work item ${item.id} slug fetch failed: ${errorMsg(error)}`,
          );
          continue;
        }
        ...
        if (!slug) {
          logger.error(
            `Plane work item ${item.id} is missing or has empty slug value.`,
          );
          continue;
        }
        ...
        if (options.saveToDisk) {
          try {
            ...
            logger.error(
              `[plane-posts-loader] failed to save ${slug}: ${errorMsg(error)}`,
            );
          }
        }
...
function missingEnv(options: PlanePostsLoaderOptions): string[] {
  ...
  if (!options.apiToken) missing.push("PLANE_API_TOKEN");
  return missing;
}
